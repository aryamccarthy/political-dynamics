...
dem_pca = pipeline.fit_transform(dem)
dem_scaled = scaler_pipeline.transform(dem)
...
def biplot(pca, dat, title='', show_points=True, components=(0, 1)):
    ...
    plt.show()
biplot(pca, pd.DataFrame(dem_scaled, columns=df.columns), title='Biplot for liberals', components=(0, 1))
