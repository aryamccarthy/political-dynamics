...
demography = pd.read_csv("../data/processed/{}_demography.csv".format(YEAR), index_col=0)
...
def make_weights_matrix(weights, X):
    ...
    return w_new
w0 = make_weights_matrix(weights, demography)
w = w0[weights.values.ravel() != 0]


# In[13]:

w0.shape, w.shape, demography[weights.values.ravel() != 0].shape, demography.shape
...
data_pca = pipeline.fit_transform(demography)
_scaled = scaler_pipeline.transform(demography)
...
(df.PartyID - 4).value_counts()


# In[24]:

data_scaled = pd.DataFrame(_scaled, columns=demography.columns)
triplot(pca, data_scaled, title='ANES {} Biplot'.format(YEAR), color=df.PartyID-4)


# In[25]:

biplot(pca, data_scaled, title='ANES {} Biplot'.format(YEAR), color=df.PartyID-4)
